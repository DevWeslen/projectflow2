...
export function StatusReport({ projectId }: StatusReportProps) {
  ...
  return (
    <>
      ...
      <div className="status-report-container bg-white text-black p-4 min-h-screen font-sans print:p-2 print:m-0 print:min-h-0 flex flex-col gap-3">
        ...
        {/* MAIN CONTENT */}
        <div className="flex flex-col md:flex-row flex-1 gap-3 min-h-0 pb-4 print:grid print:grid-cols-4 print:gap-2">

          {/* LEFT: Gantt (screen) + Pontos de Atenção */}
          <div className="flex-[2] flex flex-col gap-3 min-w-0 min-h-0 print:col-span-1 print:block">

            {/* Gantt Chart - screen only */}
            <Card className={cn(
              "p-3 border-2 border-black rounded-lg shadow-none overflow-hidden flex flex-col print:hidden bg-white transition-all duration-300",
              isFullscreen ? "flex-[2] min-h-[300px] opacity-0 pointer-events-none" : "flex-[2] min-h-[300px]"
            )}>
              <div className="flex items-center justify-between mb-2">
                <div className="w-6" />
                <h3 className="text-center font-black uppercase text-xs">Cronograma</h3>
                <Button
                  variant="ghost"
                  size="icon"
              ...
              <div className="w-full flex-1 min-h-0 overflow-x-auto relative">
                <GanttChart tasks={projectTasks} dependencies={taskDependencies} startDate={ganttStart} endDate={ganttEnd} />
              </div>
            </Card>

            {/* Fullscreen Gantt Overlay */}
            {isFullscreen && (
              <div className="fixed inset-0 z-[9999] bg-black/60 flex items-center justify-center p-4 print:hidden">
                <div className="bg-white w-full h-full rounded-xl border-2 border-black shadow-2xl flex flex-col p-4 overflow-hidden">
                  <div className="flex items-center justify-between mb-3">
                    <div className="w-8" />
                    <h3 className="text-center font-black uppercase text-sm">Cronograma</h3>
                    <Button
                      variant="ghost"
                      size="icon"
          ...
          {/* RIGHT: Lists */}
          <div className="flex-1 flex flex-col gap-3 min-w-0 min-h-0 print:col-span-3 print:grid print:grid-cols-3 print:gap-2">
            <Card className="flex-1 p-3 border-2 border-black rounded-lg shadow-none flex flex-col min-h-0">
      ...
      </div>{/* END status-report-container */}

      {/* PAGE 2: Gantt exclusivo para impressão - FORA do container para respeitar page-break-after */}
      <div className="print-gantt-page">
        <div className="w-full h-full border-2 border-black rounded-lg overflow-hidden flex flex-col bg-white p-3">
